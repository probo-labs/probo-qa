...
export default function IndexHeader() {
  ...
  const handleFillCache = useCallback(async (testScenarioIds?: string[]) => {
    setIsFilling(true);
    const scenarioIds = testScenarioIds || getAllScenarioIds();
  ...
  // Expose handleFillCache for testing purposes
  useEffect(() => {
    if (typeof window !== 'undefined') {
      window.__testFillCache = handleFillCache;
    }
    return () => {
      if (typeof window !== 'undefined') {
        delete window.__testFillCache;
      }
    };
  }, [handleFillCache]);
